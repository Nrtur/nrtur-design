...
|---|---|---|---|---|---|---|
| **nrtur** | `pipeline` + `stageKey` on deal + `stagePlacements[]` (D3) | ✅ | unlimited (prototype) | ❌ today — D3 array is the multi-placement shape, unactivated | ✅ auto: keeps stage if key exists, else enters first stage, history stamped | ❌ (backlog: tunnel recipe) |
...
**Still behind (honest backlog):** per-pipeline permissions (Bitrix24/Streak/Attio lead) · pipeline referenced by name-string not id · creation into a gated stage is ungated · no close-date forecast kanban (Pipedrive) · no cross-pipeline automation (Bigin Connected Pipelines / Bitrix24 tunnels) · multi-placement (D2 phase 2) designed but not activated · `includedForConversion` on lost reasons not implemented · reason-list edits don't rewrite past closes (market-standard, but no merge tool either).
