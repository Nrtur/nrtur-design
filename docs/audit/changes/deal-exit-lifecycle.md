...
- **Multi-pipeline per-placement close (A4)** — a pre-existing defect: marking a deal terminal on a *secondary* (enrolled) pipeline stamps a **global** `outcome`, so `dealClosedKind` reports the whole deal closed while its primary placement still sits open. Fix = only stamp deal-level `outcome` when the move is on the primary board; stamp the placement otherwise. Deferred as a separate, higher-risk pass (touches `applyMove`/the outcome modal in the most fragile board region) needing its own verification.
