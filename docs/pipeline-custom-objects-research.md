...
>
> **✅ Built (one object-selectable Pipeline page, this branch):** the Pipeline page now has an **Object** selector in its toolbar (`Deals` / `Leads` / `Companies` / `Contacts` / + every custom object). One page, not a board scattered per object. `Deals` keeps the full dedicated pipeline; selecting any other object early-returns a generic **`ObjectBoard`** (defined above `PipelinePage`) that renders through the shared `KanbanBoard`: it resolves a per-object descriptor (records source, the natural status field + its ordered options, a group-by picker, the patch path) and supports drag-to-reassign — `setLeads`/`setContacts`/`setCompanies` for built-ins (deriving `ownerColor` on owner moves) and `updateCustomRecord` for custom objects. Built-in status fields use their full ordered option list so empty stages still render as drop targets. Verified with 0 console errors: Leads → New/Contacted/Nurturing/Qualified/Unqualified (a drag re-assigned a lead's status), Companies → Customer/Prospect/Partner/Vendor, Contacts → Prospect/Customer/Lost, the Projects custom object → its Stage options; switching back to Deals restores the rich pipeline (won/lost + edit-stages intact). The `goTo('pipeline',{objectType})` payload seeds the initial object, so other pages can deep-link straight to an object's board.
