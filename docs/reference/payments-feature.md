...
2. **You can't actually pay end-to-end.** The "payment link" is a **dead URL string** — there's no customer-facing checkout page anywhere. The only way an invoice becomes "paid" is the *seller* clicking a button. → Add a simulated hosted checkout page the link/invoice opens to.
   **✅ Fixed (this branch):** a public, branded **hosted checkout page** (`PayCheckoutPage`, route `pay-checkout`, full-screen like sign-in) now exists. **Open** buttons on the invoice's payment-link row and on every payment-link row launch it. It shows the merchant, amount, line items (+ discount/tax) and a simulated card form (test card 4242…); on **Pay**, the *customer* marks the invoice paid (or bumps the link's paid count) directly via the store. Verified end-to-end: paying INV-1040 from checkout flipped it to **Paid** and lifted **Collected $3,566 → $4,038**; the link checkout paid too. *(In the prototype the link "opens" via in-app `goTo`; a deployed build would map the real `nrtur.app/pay/…` URL to this page.)*
...
5. **Invoices are immutable.** No edit, void, delete, duplicate, resend, or download-PDF — a draft with a typo can only be sent, never fixed. (Products and subscriptions have full menus; invoices, the most important object, have the least.)
   **✅ Fixed (this branch):** **Void** (finding #10), plus **Edit** (reuses the invoice drawer in edit-mode — loads the line items / discount / tax / contact / deal, "Save changes" updates in place, no duplicate), **Duplicate** (one-click copy as a fresh draft), and **Download** (generates a real printable invoice document and downloads it; a production build would render a server-side PDF). Edit verified end-to-end: a note edit persisted on reopen with no duplicate.
...
9. **No tax, discount, or currency** anywhere — the invoice total is a bare sum of qty × price, and "$"/USD is hardcoded. Tax-on-invoice is table-stakes for billing real customers.
   **✅ Fixed (this branch):** the invoice drawer now has **Discount ($)** and **Tax (%)** fields with a live **Subtotal → Discount → Tax → Total** breakdown (stored as `subtotal`/`discount`/`taxRate`/`tax`), plus a **Currency** selector (USD/EUR/GBP/CAD/AUD). `payMoney` is currency-aware, and an invoice renders consistently in its own currency across the drawer, list, detail, and checkout. *(The headline KPIs — Collected/Outstanding/MRR — still total in the workspace base currency; correct mixed-currency rollups would need FX rates, out of scope for the prototype.)*
...
11. **Payment-link "Paid" status is broken** — nothing in the UI can set it, and the menu's "Disable" can overwrite it into a stuck state. (Links are reusable; "paid" doesn't belong as a status.)
    **✅ Fixed (this branch):** `paid` is removed from the payment-link status set (`PAY_LINK_ST` → just Active / Disabled), and the one demo link that used it is reseeded to Active. Links are reusable; how many times a link was paid stays in the `{n} paid` counter. Verified: no link shows a "Paid" pill.
...
13. **"Connect Stripe" is decorative** — invoices can be marked paid whether Stripe is "connected" or not, and the account number is hardcoded.
    **◑ Partly fixed (this branch):** the Stripe connection now gates the real collection point — the customer **checkout's Pay button is disabled (with a "merchant hasn't finished connecting Stripe" notice) when Stripe isn't connected**. (Default in the prototype is connected, so the demo flows still work — verified the checkout pays when connected.) *(The hardcoded `acct_…` is demo data; seller-side mark-paid is intentionally left ungated so drafts/admin still work offline.)*
14. **Free-typed "Bill to"** lets you type a name that isn't a real contact, which silently drops the company and the timeline link (while the toast still claims it logged to the timeline).
    **✅ Fixed (this branch):** the invoice drawer now shows an inline amber note — *"Not a saved contact — this invoice won't link to a contact record"* — when the typed Bill-to isn't a known contact, and the create toast only claims *"logged to <contact>'s timeline"* when the bill-to actually matches a saved contact. (The no-note path is verified for empty + matched contacts; the note is the boolean negation of that same check.)
...
**Medium:** ⑤ ✅ simulated hosted checkout page *(done — this branch; unlocks real end-to-end pay)* · ⑥ ◑ invoice **Void / Refund + receipt** *(done — this branch; Edit / duplicate / PDF still open)* · ⑦ ✅ tax + discount + multi-currency *(done — this branch)* · ⑧ ✅ pay-at-booking creates a paid invoice *(done — this branch)*.
