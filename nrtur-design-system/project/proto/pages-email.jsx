...
function EmailInboxPage({ goTo }) {
  ...
  return (
    <div className="h-full flex" style={{background:'#09091a'}}>
        ...
        <AppTopbar goTo={goTo} title="Email" subtitle="2 unread · 3 accounts connected"
          rightSlot={
            <div className="flex items-center gap-2">
              ...
              <button onClick={()=>setComposeOpen(true)} className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-[13px] font-semibold text-white bg-brand-500 hover:bg-brand-400 shadow-brand hover:-translate-y-0.5 transition-all">
                <I.Edit size={13}/>Compose
              </button>
            </div>
          }
        />
      ...
      {composeOpen && <EmailComposeModal close={()=>setComposeOpen(false)}/>}
    </div>
  );
}

// ── Email Compose Modal ───────────────────────────────────────────────────────
function EmailComposeModal({ close, defaultTo='' }) {
  ...
  const handleSend = () => {
    setSending(true);
    setTimeout(() => { setSending(false); close(); }, 1200);
  };

  return (
    <div className="fixed inset-0 z-[150] flex items-end justify-end p-6 pointer-events-none">
      <div className="pointer-events-all w-[520px] rounded-2xl border border-white/[0.10] overflow-hidden"
           style={{background:'rgba(13,13,26,0.98)', backdropFilter:'blur(24px)', boxShadow:'0 0 0 1px rgba(255,255,255,0.05), 0 -8px 60px rgba(0,0,0,0.6)'}}>
        {/* Title bar */}
        <div className="flex items-center justify-between px-5 py-3 border-b border-white/[0.07]" style={{background:'rgba(255,255,255,0.02)'}}>
          <p className="text-[13px] font-semibold text-white">New message</p>
          <div className="flex items-center gap-1">
            <button onClick={close} className="w-7 h-7 rounded-lg flex items-center justify-center text-white/40 hover:text-white hover:bg-white/[0.07] transition-colors"><I.X size={13}/></button>
          </div>
        </div>
        {/* Fields */}
        <div className="flex flex-col divide-y divide-white/[0.05]">
          <div className="flex items-center px-5 py-2.5">
            <span className="text-[12px] text-white/40 w-14 shrink-0">To</span>
          ...
          <div className="flex items-center px-5 py-2.5">
            <span className="text-[12px] text-white/40 w-14 shrink-0">Subject</span>
            <input value={subject} onChange={e=>setSubject(e.target.value)} placeholder="Subject line…"
              className="flex-1 bg-transparent outline-none text-[13px] text-white placeholder:text-white/25"/>
          </div>
          <textarea value={body} onChange={e=>setBody(e.target.value)} rows={10}
            placeholder="Write your message…"
            className="w-full bg-transparent outline-none resize-none px-5 py-4 text-[13px] text-white placeholder:text-white/25 leading-relaxed"/>
        </div>
        {/* Toolbar */}
        <div className="flex items-center justify-between px-5 py-3 border-t border-white/[0.07]" style={{background:'rgba(255,255,255,0.01)'}}>
          <div className="flex items-center gap-1">
            {[{icon:I.Download,t:'Attach'},{icon:I.Hash,t:'Link'},{icon:I.Sparkles,t:'AI draft'}].map(({icon:Ic,t})=>(
              <button key={t} title={t} className="w-8 h-8 rounded-lg flex items-center justify-center text-white/35 hover:text-white hover:bg-white/[0.06] transition-colors">
                <Ic size={13}/>
              </button>
            ))}
          </div>
          <div className="flex items-center gap-2">
            <button onClick={close} className="px-3 py-1.5 rounded-lg text-[12px] text-white/55 hover:text-white hover:bg-white/[0.06] transition-colors">Discard</button>
            <button onClick={handleSend} disabled={!to.trim()||!subject.trim()||sending}
              className={['inline-flex items-center gap-2 px-4 py-1.5 rounded-lg text-[12px] font-semibold transition-all border-none',
                (to.trim()&&subject.trim()&&!sending)?'bg-brand-500 hover:bg-brand-400 text-white shadow-brand cursor-pointer':'bg-white/[0.06] text-white/30 cursor-not-allowed'].join(' ')}>
              <I.Send size={11} stroke={2.4}/>{sending?'Sending…':'Send'}
            </button>
          </div>
        </div>
      </div>
...
// ── SMS Inbox Page ─────────────────────────────────────────────────────────────
const SMS_CONVOS = [
  {id:1, contact:'Sarah Chen',   avatar:'SC', color:'#3b82f6', preview:'Can we confirm Thursday 2pm PT?',     time:'Just now',  unread:true,
    messages:[
      {from:'Sarah Chen', me:false, body:'Hey Alex — quick question, can we confirm Thursday 2pm PT for the onboarding call?', time:'12:41 PM'},
      {from:'Alex Morgan',me:true,  body:'Hey Sarah! Absolutely, Thursday 2pm PT works great. I\'ll send over a Zoom link now.', time:'12:43 PM'},
      {from:'Sarah Chen', me:false, body:'Perfect, thank you!', time:'12:44 PM'},
      {from:'Sarah Chen', me:false, body:'Can we confirm Thursday 2pm PT?', time:'1:02 PM'},
    ]},
  {id:2, contact:'James Rivera',  avatar:'JR', color:'#8b5cf6', preview:'Signed contract attached ✓',         time:'2h ago',   unread:true,
    messages:[
      {from:'James Rivera',me:false,body:'Hey, just wanted to let you know we signed the contract and emailed it over.', time:'10:30 AM'},
      {from:'Alex Morgan', me:true, body:'Amazing! Got it — welcome aboard James 🎉', time:'10:35 AM'},
      {from:'James Rivera',me:false,body:'Signed contract attached ✓', time:'10:36 AM'},
    ]},
  {id:3, contact:'Priya Nair',    avatar:'PN', color:'#a855f7', preview:'When does onboarding start?',         time:'1d ago',   unread:false,
    messages:[
      {from:'Priya Nair',  me:false, body:'Hi! Our team is very excited. When exactly does the onboarding process start?', time:'Yesterday 3:15 PM'},
      {from:'Alex Morgan', me:true,  body:'Hi Priya! We\'ll kick off next Monday. You\'ll get a welcome email with a setup guide.', time:'Yesterday 4:00 PM'},
      {from:'Priya Nair',  me:false, body:'When does onboarding start?', time:'Yesterday 4:02 PM'},
    ]},
  {id:4, contact:'Marcus Rios',   avatar:'MR', color:'#f59e0b', preview:'Got your voicemail, calling back',    time:'3d ago',   unread:false,
    messages:[
      {from:'Marcus Rios', me:false, body:'Got your voicemail, will call you back in 30 mins.', time:'3 days ago 2:10 PM'},
    ]},
];

function SMSInboxPage({ goTo }) {
  const [selected, setSelected] = React.useState(SMS_CONVOS[0]);
  const [draft, setDraft] = React.useState('');
  const MAX_CHARS = 160;

  return (
    <div className="h-full flex" style={{background:'#09091a'}}>
      <AppSidebar active="sms-inbox" goTo={goTo}/>
      <div className="flex-1 flex flex-col min-w-0">
        <AppTopbar goTo={goTo} title="SMS Inbox" subtitle="4 conversations · 2 unread"
          rightSlot={
            <button onClick={()=>goTo('settings-integrations')} className="inline-flex items-center gap-2 px-3 py-2 rounded-xl text-[13px] font-medium text-white/70 hover:text-white bg-white/[0.04] border border-white/[0.08] hover:border-white/[0.18] transition-all">
              <I.Plug size={13}/>Configure SMS
            </button>
          }
        />
        <div className="flex-1 flex min-h-0">
          {/* Conversation list */}
          <div className="w-[300px] shrink-0 border-r border-white/[0.05] overflow-y-auto scroll-area">
            {SMS_CONVOS.map(c=>(
              <button key={c.id} onClick={()=>setSelected(c)}
                className={['w-full text-left px-4 py-4 border-b border-white/[0.03] transition-all relative',
                  selected?.id===c.id?'bg-brand-500/[0.08] border-l-2 border-brand-500':'hover:bg-white/[0.02]'].join(' ')}>
                {c.unread && <span className="absolute left-2 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-brand-400"/>}
                <div className="flex items-start gap-3 pl-2">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between mb-0.5">
                      <p className={['text-[13px] leading-tight', c.unread?'font-bold text-white':'font-medium text-white/80'].join(' ')}>{c.contact}</p>
                      <span className="text-[10px] text-white/35 shrink-0 ml-2">{c.time}</span>
                    </div>
                    <p className={['text-[12px] truncate', c.unread?'text-white/70':'text-white/45'].join(' ')}>{c.preview}</p>
                  </div>
                </div>
              </button>
            ))}
          ...
          {selected ? (
            <div className="flex-1 flex flex-col min-w-0">
              {/* Header */}
              <div className="shrink-0 px-6 py-4 border-b border-white/[0.05] flex items-center justify-between" style={{background:'#09091a'}}>
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-full flex items-center justify-center text-[11px] font-bold text-white" style={{background:selected.color}}>{selected.avatar}</div>
                  <div>
                    <p className="text-[14px] font-bold text-white">{selected.contact}</p>
                    <p className="text-[11px] text-white/45">SMS conversation</p>
                  </div>
                </div>
                <button onClick={()=>goTo('contact-detail')} className="text-[12px] text-brand-300 hover:text-brand-200 flex items-center gap-1">View contact<I.ArrowRight size={11} stroke={2.4}/></button>
              </div>
              ...
              {/* Messages */}
              <div className="flex-1 overflow-y-auto scroll-area px-6 py-5 flex flex-col gap-3">
                {selected.messages.map((msg,i)=>(
                  <div key={i} className={['flex', msg.me?'justify-end':'justify-start'].join(' ')}>
                    <div className={['max-w-[70%] px-4 py-2.5 rounded-2xl text-[13px] leading-relaxed',
                      msg.me?'bg-brand-500 text-white rounded-br-sm':'bg-white/[0.07] border border-white/[0.08] text-white/85 rounded-bl-sm'].join(' ')}>
                      <p>{msg.body}</p>
                      <p className={['text-[10px] mt-1', msg.me?'text-white/60':'text-white/35'].join(' ')}>{msg.time}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {/* Compose bar */}
              <div className="shrink-0 border-t border-white/[0.05] p-4" style={{background:'#09091a'}}>
                <div className="rounded-xl border border-white/[0.08] focus-within:border-brand-500/40 focus-within:shadow-[0_0_0_3px_rgba(99,102,241,0.10)] transition-all" style={{background:'rgba(15,15,28,0.6)'}}>
                  <textarea value={draft} onChange={e=>setDraft(e.target.value.slice(0,MAX_CHARS))} rows={3}
                    placeholder="Type an SMS message…"
                    className="w-full bg-transparent outline-none resize-none px-4 pt-3 pb-1 text-[13px] text-white placeholder:text-white/30"/>
                  <div className="flex items-center justify-between px-4 pb-3">
                    <span className={['text-[11px] tabular-nums', draft.length>140?'text-amber-300':'text-white/30'].join(' ')}>
                      {draft.length}/{MAX_CHARS}
                    </span>
                    <button disabled={!draft.trim()} onClick={()=>setDraft('')}
                      className={['inline-flex items-center gap-2 px-4 py-1.5 rounded-lg text-[12px] font-semibold transition-all border-none',
                        draft.trim()?'bg-brand-500 hover:bg-brand-400 text-white shadow-brand cursor-pointer':'bg-white/[0.06] text-white/30 cursor-not-allowed'].join(' ')}>
                      <I.Send size={11} stroke={2.4}/>Send SMS
                    </button>
                  </div>
                </div>
              </div>
            ...
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                <div className="w-12 h-12 rounded-xl bg-white/[0.03] border border-white/[0.06] flex items-center justify-center mx-auto mb-4 text-white/25"><I.Hash size={22}/></div>
                <p className="text-[14px] font-semibold text-white/50">Select a conversation</p>
              </div>
            </div>
          )}
        </div>
        <AppFooter note="Twilio · +1 415-555-0100"/>
      </div>
    </div>
  );
}

Object.assign(window, { EmailInboxPage, EmailComposeModal, SMSInboxPage });
