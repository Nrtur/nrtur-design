...
// ── Contacts Filter Panel (right drawer) ──────────────────────────────────────
function ContactsFilterPanel({ close, onApply }) {
  ...
  const STATUSES = ['Active','Hot','Warm','New','Lead','Qualified'];
  const TAGS     = ['agency','top-lead','q2-focus','new-lead','won','qualified'];
  const OWNERS   = ['All owners','Alex Morgan','Sarah Chen','Jamie Kim','Ravi Lee'];
  const LAST     = ['Any time','Today','This week','This month','Over 30 days ago','Over 90 days ago'];

  const activeCount = status.length + tags.length + (owner?1:0) + (lastContact?1:0);
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div className="fixed right-0 top-0 bottom-0 z-[100] w-72 flex flex-col border-l border-white/[0.08] shadow-[−8px_0_40px_rgba(0,0,0,0.5)]"
           style={{background:'rgba(11,11,24,0.98)', backdropFilter:'blur(16px)'}}>
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.06]">
          <div className="flex items-center gap-2">
            <p className="text-[14px] font-bold text-white">Filters</p>
            {activeCount > 0 && (
              <span className="px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-brand-500 text-white">{activeCount}</span>
            )}
          </div>
          <button onClick={close} className="text-white/40 hover:text-white transition-colors"><I.X size={15}/></button>
        </div>

        <div className="flex-1 overflow-y-auto scroll-area px-5 py-4 flex flex-col gap-6">
          {/* Status */}
          <div>
            <p className="text-[11px] font-semibold text-white/45 uppercase tracking-[0.14em] mb-3">Status</p>
            <div className="flex flex-wrap gap-2">
              {STATUSES.map(s => (
                <button key={s} onClick={() => toggleArr(status, setStatus, s)}
                  className={['px-3 py-1.5 rounded-lg text-[12px] font-medium border transition-all',
                    status.includes(s) ? 'bg-brand-500/15 text-brand-200 border-brand-500/30' : 'bg-white/[0.04] text-white/55 border-white/[0.08] hover:border-white/[0.18]'].join(' ')}>
                  {s}
                </button>
              ))}
            </div>
          ...
          {/* Tags */}
          <div>
            <p className="text-[11px] font-semibold text-white/45 uppercase tracking-[0.14em] mb-3">Tags</p>
            <div className="flex flex-wrap gap-2">
              {TAGS.map(t => (
                <button key={t} onClick={() => toggleArr(tags, setTags, t)}
                  className={['px-3 py-1.5 rounded-lg text-[12px] font-medium border transition-all',
                    tags.includes(t) ? 'bg-brand-500/15 text-brand-200 border-brand-500/30' : 'bg-white/[0.04] text-white/55 border-white/[0.08] hover:border-white/[0.18]'].join(' ')}>
                  {t}
                </button>
              ))}
            </div>
          ...
          {/* Owner */}
          <div>
            <p className="text-[11px] font-semibold text-white/45 uppercase tracking-[0.14em] mb-2">Owner</p>
            <select value={owner} onChange={e => setOwner(e.target.value)}
              className="w-full px-3 py-2.5 rounded-xl bg-white/[0.04] border border-white/[0.08] text-[13px] text-white/80 outline-none focus:border-brand-500/40 appearance-none">
          ...
          {/* Last contacted */}
          <div>
            <p className="text-[11px] font-semibold text-white/45 uppercase tracking-[0.14em] mb-2">Last contacted</p>
            <select value={lastContact} onChange={e => setLastContact(e.target.value)}
              className="w-full px-3 py-2.5 rounded-xl bg-white/[0.04] border border-white/[0.08] text-[13px] text-white/80 outline-none focus:border-brand-500/40 appearance-none">
              <option value="">Any time</option>
              {LAST.slice(1).map(l => <option key={l} value={l}>{l}</option>)}
            </select>
          </div>
        </div>
        ...
        <div className="px-5 py-4 border-t border-white/[0.06] flex gap-3">
          <button onClick={() => { setStatus([]); setTags([]); setOwner(''); setLastContact(''); }}
            className="flex-1 py-2.5 rounded-xl text-[13px] font-medium text-white/55 bg-white/[0.04] border border-white/[0.08] hover:bg-white/[0.07] transition-all">
            Clear all
          </button>
          <button onClick={() => { onApply?.({status,tags,owner,lastContact}); close(); }}
            className="flex-1 py-2.5 rounded-xl text-[13px] font-bold text-white bg-brand-500 hover:bg-brand-400 shadow-brand border-none transition-all">
            Apply
          </button>
        </div>
      </div>
...
function DuplicateDetectionModal({ email, close, onKeepBoth, onMerge }) {
  const existing = {name:'Sarah Chen', email:'sarah@meridian.co', co:'Meridian Agency', status:'Active', last:'2h ago', deal:'$24k'};
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center" style={{background:'rgba(0,0,0,0.72)', backdropFilter:'blur(8px)'}}>
      <div style={{width:560, background:'rgba(13,13,26,0.97)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:16, backdropFilter:'blur(24px)', boxShadow:'0 0 0 1px rgba(255,255,255,0.04), 0 32px 80px rgba(0,0,0,0.65)', padding:24}}>
        <div className="flex items-start gap-4 mb-6">
          <div className="w-10 h-10 rounded-xl bg-amber-500/10 ring-1 ring-amber-500/25 flex items-center justify-center shrink-0">
            <I.Users size={17} sx={{color:'#fbbf24'}}/>
          </div>
          <div>
            <p className="text-white text-[15px] font-bold">Possible duplicate detected</p>
            <p className="text-white/55 text-[13px] mt-1">A contact with email <span className="text-white font-medium">{email}</span> already exists in your workspace.</p>
          </div>
        </div>

        {/* Side-by-side comparison */}
        <div className="grid grid-cols-2 gap-3 mb-6">
          {[{label:'Existing contact', data:existing, accent:true},
            {label:'New contact',      data:{name:'Sarah Chen', email, co:'Meridian Agency', status:'New', last:'Now', deal:'—'}, accent:false}
          ].map(({label, data, accent}) => (
            <div key={label} className={['rounded-xl p-4 border', accent ? 'bg-brand-500/08 border-brand-500/25' : 'bg-white/[0.03] border-white/[0.08]'].join(' ')}>
              <p className={['text-[10px] font-semibold uppercase tracking-[0.14em] mb-3', accent ? 'text-brand-300' : 'text-white/40'].join(' ')}>{label}</p>
              <div className="flex items-center gap-2 mb-3">
                <div className="w-8 h-8 rounded-full bg-blue-500 flex items-center justify-center text-[10px] font-bold text-white shrink-0">SC</div>
                <p className="text-[13px] font-semibold text-white">{data.name}</p>
              </div>
              {[['Email',data.email],['Company',data.co],['Status',data.status],['Last seen',data.last],['Deal',data.deal]].map(([k,v]) => (
                <div key={k} className="flex items-start gap-2 mb-1.5">
                  <span className="text-[11px] text-white/35 w-16 shrink-0">{k}</span>
        ...
        <div className="flex gap-3">
          ...
          <button onClick={onKeepBoth||close} className="flex-1 py-2.5 rounded-xl text-[13px] font-semibold text-white/80 bg-white/[0.06] border border-white/[0.10] hover:bg-white/[0.09] transition-all">Keep both</button>
          <button onClick={onMerge||close} className="flex-1 py-2.5 rounded-xl text-[13px] font-bold text-white bg-brand-500 hover:bg-brand-400 shadow-brand border-none transition-all">Merge contacts</button>
        </div>
      </div>
    </div>
  );
...
function ContactsPage({ goTo }) {
  ...
  const COL = '36px 1.8fr 1.4fr 110px 110px 90px 110px 36px';
    ...
    <div className="h-full flex" style={{background:'#09091a'}}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="shrink-0 border-b border-white/[0.05] px-7 py-3 flex items-center gap-3" style={{background:'#09091a'}}>
          ...
          <div className="ml-auto flex items-center gap-2">
            <button onClick={()=>setFilterOpen(true)}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] text-white/55 hover:text-white border border-white/[0.06] hover:border-brand-500/30 hover:bg-brand-500/08 transition-all">
        ...
        <div className="flex-1 overflow-y-auto scroll-area">
          ...
          {selected.length > 0 && (
            <div className="sticky top-0 z-20 px-7 py-2.5 flex items-center gap-3 border-b border-brand-500/25"
                 style={{background:'rgba(99,102,241,0.10)', backdropFilter:'blur(12px)'}}>
              <span className="text-[13px] font-semibold text-brand-200">{selected.length} selected</span>
              <div className="flex items-center gap-2 ml-2">
                {[
                  {l:'Tag',          Icon:I.Flag},
                  {l:'Email',        Icon:I.Mail},
                  {l:'SMS',          Icon:I.Hash},
                  {l:'Change status',Icon:I.CheckSquare},
                ].map(({l,Icon})=>(
                  <button key={l} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-medium text-white/75 hover:text-white bg-white/[0.06] hover:bg-white/[0.10] border border-white/[0.10] transition-all">
                    <Icon size={12}/>{l}
                  </button>
                ))}
                <button onClick={()=>openConfirm({
                  title:`Delete ${selected.length} contact${selected.length>1?'s':''}?`,
                  body:'Selected contacts will be archived for 30 days and can be restored.',
                  confirmLabel:'Delete selected', danger:true,
                  onConfirm:()=>setSelected([]),
                })} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-medium text-red-300 hover:text-red-200 bg-red-500/10 hover:bg-red-500/15 border border-red-500/20 transition-all">
                  <I.Trash size={12}/>Delete
                </button>
              </div>
              <button onClick={()=>setSelected([])} className="ml-auto text-white/40 hover:text-white transition-colors"><I.X size={14}/></button>
            </div>
          )}
          ...
          <div className="sticky top-0 z-10 grid items-center gap-4 px-7 py-3 border-b border-white/[0.05] text-[10px] font-semibold text-white/40 uppercase tracking-[0.14em]"
            ...
            <button onClick={toggleAll}
              className={['w-4 h-4 rounded flex items-center justify-center border transition-all shrink-0',
                allChecked?'bg-brand-500 border-brand-500':'bg-transparent border-white/25 hover:border-white/50'].join(' ')}>
              {allChecked && <I.Check size={9} stroke={3} sx={{color:'white'}}/>}
            </button>
            <span>Name</span><span>Company</span><span>Status</span><span>Last seen</span><span>Owner</span><span>Deal value</span><span/>
          </div>
          {/* Rows */}
          ...
          {filtered.map((c,i)=>(
            <div key={c.id}
              className={['w-full grid items-center gap-4 px-7 py-3.5 hover:bg-white/[0.025] transition-colors group',
                i<filtered.length-1?'border-b border-white/[0.03]':'',
                selected.includes(c.id)?'bg-brand-500/[0.04]':''].join(' ')}
              style={{gridTemplateColumns:COL}}>
              {/* Checkbox */}
              <button onClick={()=>toggleSelect(c.id)}
                className={['w-4 h-4 rounded flex items-center justify-center border transition-all shrink-0',
                  selected.includes(c.id)?'bg-brand-500 border-brand-500':'bg-transparent border-white/20 hover:border-white/50 opacity-0 group-hover:opacity-100'].join(' ')}>
                {selected.includes(c.id) && <I.Check size={9} stroke={3} sx={{color:'white'}}/>}
              </button>
              {/* Rest of row — click to navigate */}
              <button onClick={()=>goTo('contact-detail')} className="contents">
        ...
        {filterOpen && <ContactsFilterPanel close={()=>setFilterOpen(false)}/>}
        {confirmNode}
...
function AddContactPage({ goTo }) {
  ...
  const KNOWN_EMAILS = ['sarah@meridian.co', 'james@summit.co', 'maria@bloom.co'];

  const handleCreate = () => {
    if (KNOWN_EMAILS.includes(email.toLowerCase().trim())) {
      setDupModal(true);
    } else {
      goTo('contact-detail');
    }
    ...
    <div className="h-full flex" style={{background:'#07070f'}}>
          ...
          <div className="px-7 py-6 max-w-2xl mx-auto">
            <div className="rounded-2xl bg-white/[0.03] border border-white/[0.06] p-7">
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                {[['First name','text'],['Last name','text'],['Company','text'],['Job title','text']].map(([pl,t])=>(
                  <div key={pl}><label className="text-[11px] font-semibold text-white/45 uppercase tracking-[0.14em] block mb-1.5">{pl}</label>
                    <input type={t} placeholder={pl} className="w-full px-3 py-2.5 rounded-xl bg-white/[0.04] border border-white/[0.10] text-[13px] text-white placeholder:text-white/25 outline-none focus:border-brand-500/40"/>
                  </div>
                ))}
              </div>
              <div className="mb-4">
                <label className="text-[11px] font-semibold text-white/45 uppercase tracking-[0.14em] block mb-1.5">Work email</label>
                <input type="email" value={email} onChange={e=>setEmail(e.target.value)}
                  placeholder="email@company.com"
                  className="w-full px-3 py-2.5 rounded-xl bg-white/[0.04] border border-white/[0.10] text-[13px] text-white placeholder:text-white/25 outline-none focus:border-brand-500/40"/>
              </div>
              <div className="mb-6">
                <label className="text-[11px] font-semibold text-white/45 uppercase tracking-[0.14em] block mb-1.5">Phone</label>
              ...
              <div className="flex gap-3">
                ...
                <button onClick={handleCreate} className="flex-1 py-2.5 rounded-xl text-[13px] font-bold text-white bg-brand-500 hover:bg-brand-400 shadow-brand transition-all">Create contact</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {dupModal && (
        <DuplicateDetectionModal
          email={email}
          close={()=>setDupModal(false)}
          onKeepBoth={()=>{ setDupModal(false); goTo('contacts'); }}
          onMerge={()=>{ setDupModal(false); goTo('contact-detail'); }}
        />
      )}
    </div>
  );
...
Object.assign(window, {
  ContactsFilterPanel, DuplicateDetectionModal,
  ContactsPage, ContactDetailPage, AddContactPage, EditContactPage, CONTACTS_DATA,
});
