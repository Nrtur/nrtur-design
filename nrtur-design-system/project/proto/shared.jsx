...
function AppSidebar({ active, goTo }) {
  const items = [
    ...
    { key:'calls',       Icon:I.Phone,      label:'Calls' },
    { key:'automations', Icon:I.Zap,        label:'Automations' },
    { key:'reports',     Icon:I.BarChart,   label:'Reports' },
  ];
  const inAutoSection  = ['automations','automation-builder','sms-sequences','email-sequences'].includes(active);
  const inEmailSection = ['email-inbox','sms-inbox'].includes(active);
    ...
    <aside className="w-14 shrink-0 flex flex-col items-center py-5 gap-1.5 border-r border-white/[0.05]" style={{background:'#07070f'}}>
      ...
      {items.map(({key,Icon,label})=>{
        const a = active===key
          ...
          || (key==='email-inbox' && inEmailSection);
        return (
          <React.Fragment key={key}>
            ...
            {/* Email sub-links: Email / SMS Inbox */}
            {key==='email-inbox' && inEmailSection && (
              <div className="flex flex-col gap-1">
                {[{p:'email-inbox',icon:I.Mail,t:'Email'},{p:'sms-inbox',icon:I.Hash,t:'SMS Inbox'}].map(({p,icon:SubIcon,t})=>(
                  <button key={p} onClick={()=>goTo(p)} title={t}
                    className={['w-7 h-7 rounded-lg flex items-center justify-center transition-all',
                      active===p?'bg-brand-500/20 text-brand-300 ring-1 ring-brand-500/25':'text-white/25 hover:text-white/60 hover:bg-white/[0.04]'].join(' ')}
                    style={{marginLeft:2}}>
                    <SubIcon size={11}/>
                  </button>
                ))}
            ...
            {/* Automations sub-links: Email Seq / SMS Seq */}
            {key==='automations' && inAutoSection && (
              <div className="flex flex-col gap-1">
                {[{p:'email-sequences',icon:I.Mail,t:'Email Seq'},{p:'sms-sequences',icon:I.Layers,t:'SMS Seq'}].map(({p,icon:SubIcon,t})=>(
                  <button key={p} onClick={()=>goTo(p)} title={t}
                    className={['w-7 h-7 rounded-lg flex items-center justify-center transition-all',
                      active===p?'bg-brand-500/20 text-brand-300 ring-1 ring-brand-500/25':'text-white/25 hover:text-white/60 hover:bg-white/[0.04]'].join(' ')}
                    style={{marginLeft:2}}>
...
function SettingsSubNav({ active, goTo }) {
  const tabs=[
    ...
    {label:'Privacy',       page:'settings-privacy'},
...
const ALL_PAGES = [
  ...
  'email-inbox','sms-inbox',
  'calls',
  'automations','automation-builder','email-sequences','sms-sequences',
  'reports',
  'settings-billing','settings-team','settings-general','settings-integrations',
  'settings-pipeline','settings-notifications','settings-unsubscribes','settings-privacy',
  'profile',
];
